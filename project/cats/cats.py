...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        return typed_word
    lowest_diff = min(word_list, key=lambda x: diff_function(typed_word, x, limit))
    return typed_word if diff_function(typed_word, lowest_diff, limit) > limit else lowest_diff
    # END PROBLEM 5
...
def feline_fixes(typed, source, limit):
    ...
    # BEGIN PROBLEM 6
    def helper(total, i):
        if i >= min(len(typed), len(source)) or total > limit:
            return total
        elif typed[i] != source[i]:
            return helper(total+1, i+1)
        return helper(total, i+1)
    return abs(len(typed) - len(source)) + helper(0, 0)
    # END PROBLEM 6
...
def report_progress(typed, prompt, user_id, upload):
    ...
    "*** YOUR CODE HERE ***"
    i = 0
    while i <= len(typed) - 1 and typed[i] == prompt[i]:
        i += 1
...
def time_per_word(words, times_per_player):
    ...
    for i in range(len(times_per_player)):
        ...
        for j in range(len(times_per_player[i]) - 1):
            temp += [times_per_player[i][j+1] - times_per_player[i][j]]
        times += [temp]
...
def fastest_words(match):
    ...
    for i in word_indices:
        fastest_player, fastest_time = 0, time(match, 0, i)
        for j in player_indices:
            curr_time = time(match, j, i)
            if curr_time < fastest_time:
                fastest_player, fastest_time = j, curr_time
        fastest[fastest_player] += [get_word(match, i)]
    return fastest
    # END PROBLEM 10
